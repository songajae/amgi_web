...
function EnglishStudy({ chapter }) {
  ...
  if (!currentVideo) {
    return <div className="no-content">영상이 없습니다.</div>;
  }
  ...
  return (
    <div className="english-study-container">
      ...
      <div className="video-info">
        <h3 className="video-title">{currentVideo.videoTitle}</h3>
      ...
      <div className="subtitle-list-container">
        <div className="subtitle-list">
          {pageSubtitles.map(sub => (
            <div
              key={sub.id}
              className={`subtitle-item ${Math.abs(currentTime - sub.startTime) <= 3 ? 'active' : ''}`}
              onClick={() => handleSubtitleClick(sub.startTime)}
            >
              <span className="subtitle-time">{formatTime(sub.startTime)}</span>
